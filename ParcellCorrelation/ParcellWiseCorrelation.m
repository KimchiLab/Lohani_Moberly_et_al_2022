%% This script calculate correlations (cross-correlation with specified time lag or pearson's correlation) between parcells for different color combinations (blue-blue, green-green)
%or within parcell for two colors (blue and green) during three sustained
%states (locomotion, face high, face low). All time are in seconds. Please nothe that the code includes both 
%shuffling of data (done just for visualization) and permutation of epoch labels (done for permuation statistics) 
%written by Sweyta Lohani 2022
close all; clear all
%% user defined folder inputs
figuresFolder='F:\GRABS_Data\Analyzed_SVDMethodPatch14\Figures\FiguresRevisionsJuly202021\StateSpontCrossCorrelationBothHemisphere'; %where figures will be saved
inputFolder='Z:\GRABS_Data\Analyzed_SVDMethodPatch14\DualMice';%main data directory
MiceAnalyze=[{'grabAM05\imaging with 575 excitation\'},{'grabAM06\imaging with 575 excitation\'},{'grabAM07\imaging with 575 excitation\'},{'grabAM08\imaging with 575 excitation\'},{'grabAM09\imaging with 575 excitation\'},{'grabAM10\imaging with 575 excitation\'}];%mice folders
Condition='NoDrug'; %'NoDrug','PreDrug','PostDrug', specify it's a drug or drug free session
%% user-selected input parameters
params.signalsExtraction= 'RCaMP_AC'; % 'blueuv' if you are imaging blue and uv only or 'RCaMP_AC' if you are imaging blue,green and uv
...
params.ShuffleNum=1; %number of times to shuffle data (if you want to use shuffled control for comparison)
...
%% add functions and make figures folder
addpath(genpath('.\Functions'));
if ~exist(figuresFolder),mkdir(figuresFolder); end
%% get parcell indices for parcells we care about in the left and right hemisphere (remove parcells along the midline and superior/inferior colliculi)
...
for animal=1:length(MiceAnalyze)
    ...
    for folder =1:length(DirFolders)
        ...
        %% extract imaging in selected parcells and created shuffled data for checking that correlation patterns are not present in shuffled (this is not the permutation test for stats)
        ...
        [wheelOn_final,wheelOff_final,Face_HighArousal_On_final,Face_HighArousal_Off_final,Face_LowArousal_On_final,Face_LowArousal_Off_final]...
            =getSustainedStates(timing,params,imaging_time,pupil_time,face_Norm,indivFigureFolder);
...
for i=1:length(names)
    ...
    [output.(names{i}).meanLoc,output.(names{i}).meanFaceH,output.(names{i}).meanFaceL,output.(names{i}).LocFaceHmeanDiffMat,output.(names{i}).LocFaceLmeanDiffMat,output.(names{i}).FaceHFaceLmeanDiffMat,...
        output.(names{i}).AllParLocFaceH, output.(names{i}).AllParLocFaceL, output.(names{i}).AllParFaceHFaceL]= statsCorrelation_Combined(loccorrMatCatZScore.(names{i}),FaceHighcorrMatCatZScore.(names{i}),FaceLowcorrMatCatZScore.(names{i}),...
        PermuteLocFaceH.(names{i}).state1corrMatCatZScorePerm,PermuteLocFaceH.(names{i}).state2corrMatCatZScorePerm,PermuteLocFaceL.(names{i}).state1corrMatCatZScorePerm,PermuteLocFaceL.(names{i}).state2corrMatCatZScorePerm,PermuteFaceHFaceL.(names{i}).state1corrMatCatZScorePerm,PermuteFaceHFaceL.(names{i}).state2corrMatCatZScorePerm,...
    ...
    [output.(names{i}).meanLocShuff,output.(names{i}).meanFaceHShuff,output.(names{i}).meanFaceLShuff,output.(names{i}).LocFaceHmeanDiffMatShuff,output.(names{i}).LocFaceLmeanDiffMatShuff,output.(names{i}).FaceHFaceLmeanDiffMatShuff,...
        output.(names{i}).AllParLocFaceHShuff, output.(names{i}).AllParLocFaceLShuff, output.(names{i}).AllParFaceHFaceLShuff]= statsCorrelation_Combined(loccorrMatCatZScoreShuffled.(names{i}),FaceHighcorrMatCatZScoreShuffled.(names{i}),FaceLowcorrMatCatZScoreShuffled.(names{i}),...
        PermuteLocFaceH.(names{i}).state1corrMatCatZScorePermShuff,PermuteLocFaceH.(names{i}).state2corrMatCatZScorePermShuff,PermuteLocFaceL.(names{i}).state1corrMatCatZScorePermShuff,PermuteLocFaceL.(names{i}).state2corrMatCatZScorePermShuff,PermuteFaceHFaceL.(names{i}).state1corrMatCatZScorePermShuff,PermuteFaceHFaceL.(names{i}).state2corrMatCatZScorePermShuff,...
        names{i},'locShuff','FaceHighShuff','FaceLowShuff',parcellnames,CombinedParcellIdx,V1Idx,S1bIdx,M2Idx,figFolder);
end
