...
figuresFolder='F:\GRABS_Data\Analyzed_SVDMethodPatch14\Figures\Hemodynamics\VisualStimDipMethodComparison\GFP';%where figures and output will be saved
inputFolder='Z:\GRABS_Data\Analyzed_SVDMethodPatch14\Raw and UVPix outputs for dual mice and egfp\GFP';%where pixelwise regressed and uncorrected data are
inputFolder1='Z:\GRABS_Data\Analyzed_SVDMethodPatch14\GFP';%where spatial SVd data and smrx data are
%MiceAnalyze=[{'grab05\'},{'grab06\'},{'grab07\'},{'grab08\'},{'grab09\'},{'grab10\'}];
%MiceAnalyze1=[{'grabAM05\imaging with 575 excitation\'},{'grabAM06\imaging with 575 excitation\'},{'grabAM07\imaging with 575 excitation\'},{'grabAM08\imaging with 575 excitation\'},{'grabAM09\imaging with 575 excitation\'},{'grabAM10\imaging with 575 excitation\'}];
...
for animal=1:length(MiceAnalyze)
    ...
    for folder =1:length(DirFolders)
        ...
        for rr=1:length(names)
            ...
            %get trial data for each visual stim (100% contrast)and Z-score data
            if ~isempty(timestamps.visStim)
                ...
                V1trials.vis.svdUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                ...
                preEventMean=nanmean(V1trial(1:params.baselineWin*params.fsimaging,:));
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.vis.pixUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                ...
                preEventMean=nanmean(V1trial(1:params.baselineWin*params.fsimaging,:));
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.vis.uncorrected.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
            end
            ...
            if ~isempty(timestamps.airpuff)
                ...
                preEventMean=nanmean(V1trial(1:params.baselineWin*params.fsimaging,:));
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.airpuff.svdUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                
                [V1trial,~]= TrialTimeArrangeDff(V1data.pixUV,timestamps.timaging,params.fsimaging,params.preEventWin,eventTS,params.postEventWin);
                preEventMean=nanmean(V1trial(1:params.baselineWin*params.fsimaging,:));
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.airpuff.pixUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                
                [V1trial,~]= TrialTimeArrangeDff(V1data.uncorrected,timestamps.timaging,params.fsimaging,params.preEventWin,eventTS,params.postEventWin);
                preEventMean=nanmean(V1trial(1:params.baselineWin*params.fsimaging,:));
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.airpuff.uncorrected.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
            end
        end
    end
    ...
    if isfield(V1trials,'vis')
        ...
        combinedAverage=nanmean(combinedData,2);
        postStimAverage=combinedAverage(((params.preEventWin*params.fsimaging)+1):(params.postEventWin+params.preEventWin)*params.fsimaging);
...
if isfield(minAveFlour,'vis')
    ...
    currMedian=median(finalData.vis.data.(names{rr}),1);
    plot(currMedian,'+','MarkerSize',30);  %draw markers at median
    xlim([0.7 3]);xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
    box off; ylabel('MinimumZ-Score');
    ...
    [finalData.vis.stats.permPVal.(names{rr})]=permutePairedTestUpdated(finalData.vis.data.(names{rr}),10000);
    end 
end
...
if isfield(minAveFlour,'airpuff')
    ...
    currMedian=median(finalData.airpuff.data.(names{rr}),1);
    plot(currMedian,'+','MarkerSize',30);  %draw markers at median
    xlim([0.7 3]);xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
    box off; ylabel('MinimumZ-Score');
    ...
    [finalData.airpuff.stats.permPVal.(names{rr})]=permutePairedTestUpdated(finalData.airpuff.data.(names{rr}),10000);
    end 
end
%save summary data
