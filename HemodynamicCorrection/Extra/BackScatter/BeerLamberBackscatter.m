load('F:\Sweyta\GRABS_Data\BackScatter\PrahlExtinctionCoeffs.mat');
load('F:\Sweyta\GRABS_Data\BackScatter\HillmanPathlengths.mat');
%wavelengths used
lambda_EX=473.23;
lambda_EM=519.99;
lambda_R1=577.20;
...
%path lengths in um
x_EM= interp1(hillman_pathlengths.data(:,1),hillman_pathlengths.data(:,2),lambda_EM,'pchip','extrap')./2;%need to divide the excitation wavelenght path length by half according to Waters
x_EX= interp1(hillman_pathlengths.data(:,1),hillman_pathlengths.data(:,2),lambda_EX,'pchip','extrap')./2;%need to divide the emission wavelength path length by half according to Waters
...
%% do regression with two backscatter wavelengths
coeff = regress(bluesig(ipix, :)', [bs1sig(ipix, :); bs_ratio(ipix, :)]');
al_coeff1(ipix) = coeff(1);
al_coeff2(ipix) = coeff(2);
