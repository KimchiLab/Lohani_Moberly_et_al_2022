...
figuresFolder='F:\GRABS_Data\Analyzed_SVDMethodPatch14\FiguresRevisionsFeb2022\Hemodynamics\VisualStimDipMethodComparison\GFPMcherryMice\DFF';%where figures and output will be saved
inputFolder='W:\GRABS_Data\Analyzed_SVDMethodPatch14\GFPMcherryControl';%where pixelwise regressed and uncorrected data are
...
params.duration=1; % option of 1 or 2 
...
%save summary data
save(fullfile(figuresFolder,strcat('SummaryData',params.options)),'finalData');

%make trial averaged plots (averaged across trials and then animals) for
%gfp and mcherry for V1, S1, and M2
h1=figure; title('TrialAverage')
startplot=0;
baselineTime=-(params.preEventWin-1/params.fsimaging):1/params.fsimaging:(params.postEventWin);
for rr=1:length(names)% for each color
    data_svdUV=nanmean(cat(3,combinedAverage.vis.svdUV.(names{rr}){:}),3);
    data_pixUV=nanmean(cat(3,combinedAverage.vis.pixUV.(names{rr}){:}),3);
    data_uncorrected=nanmean(cat(3,combinedAverage.vis.uncorrected.(names{rr}){:}),3);
    subplot(2,3,startplot+1);plot(baselineTime,data_uncorrected(:,V1Idx));hold on; plot(baselineTime,data_pixUV(:,V1Idx));plot(baselineTime,data_svdUV(:,V1Idx));legend({'Uncorre','PixUV','SpatialUV'});
    title(strcat('V1',names{rr}));  ylabel(params.options);ylim([-1 1]); 
    subplot(2,3,startplot+2);plot(baselineTime,data_uncorrected(:,S1bIdx));hold on; plot(baselineTime,data_pixUV(:,S1bIdx));plot(baselineTime,data_svdUV(:,S1bIdx));
    title(strcat('S1b',names{rr}));  ylabel(params.options);ylim([-1 1]); 
    subplot(2,3,startplot+3);plot(baselineTime,data_uncorrected(:,M2Idx));hold on; plot(baselineTime,data_pixUV(:,M2Idx));plot(baselineTime,data_svdUV(:,M2Idx));
    title(strcat('M2',names{rr}));  ylabel(params.options);ylim([-1 1]); 
    startplot=startplot+3; 
...
for rr=1:length(names)% for each color
    ...
    title(strcat('V1',names{rr}));  ylabel(params.options);ylim([-2 1]);
end
...
for rr=1:length(names)% for each color
    ...
    set(gca,'XDir','reverse');title(strcat('Pixelwise',params.options,names{rr})); 
    ...
    set(gca,'XDir','reverse');title(strcat('Spatial',params.options,names{rr})); 
    ...
    saveas(figure2, fullfile(figuresFolder,strcat('Pixelwise-',names{rr},params.options)));
    saveas(figure2, fullfile(figuresFolder,strcat('Pixelwise-',names{rr},params.options,'.pdf')));
    saveas(figure3, fullfile(figuresFolder,strcat('Spatial-',names{rr},params.options)));
    saveas(figure3, fullfile(figuresFolder,strcat('Spatial-',names{rr},params.options,'.pdf')));
end 
