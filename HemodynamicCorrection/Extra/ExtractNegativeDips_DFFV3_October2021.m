%% This script calculate peak of negative flourescence dips around a particular event (such as visual stim for GRABs mice or airpuff for GFP control mice)
%written by Sweyta Lohani 2020
close all; clear all
%% user defined folder inputs
figuresFolder='F:\GRABS_Data\Analyzed_SVDMethodPatch14\FiguresRevisionsOctober2021\Hemodynamics\VisualStimDipMethodComparison\DualMice';%where figures and output will be saved
inputFolder='Z:\GRABS_Data\Analyzed_SVDMethodPatch14\Raw and UVPix outputs for dual mice and egfp\DualMice';%where pixelwise regressed and uncorrected data are
inputFolder1='Z:\GRABS_Data\Analyzed_SVDMethodPatch14\DualMice';%where spatial SVd data and smrx data are
MiceAnalyze=[{'grab05\'},{'grab06\'},{'grab07\'},{'grab08\'},{'grab09\'},{'grab10\'}];%grabs/rcamp pixelwise regressed and uncorrected
...
%MiceAnalyze=[{'SL224_egfp'},{'SL225_egfp'},{'EGFP control'}]; % gfp control sessions
%MiceAnalyze1=[{'SL224_egfp'},{'SL225_egfp'},{'EGFP control'}]; % gfp control sessions
...
%%
addpath(genpath('F:\Sweyta\GRABS_Data\FinalPaperCodes\EvokedActivity'));
...
for animal=1:length(MiceAnalyze)
    ...
    for folder =1:length(DirFolders)
        ...
        for rr=1:length(names)
            ...
            if ~isempty(timestamps.visStim)
                ...
                preEventStd=nanstd(V1trial(1:params.baselineWin*params.fsimaging,:));
                V1trials.vis.svdUV.(names{rr}){folder}=(V1trial-preEventMean).*100; %mulitply by 100 to get percentages
                V1trials_ZScore.vis.svdUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                ...
                V1trials_ZScore.vis.pixUV.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
                ...
                V1trials_ZScore.vis.uncorrected.(names{rr}){folder}=(V1trial-preEventMean)./preEventStd;
            end
    ...
    for rr=1:length(names)% for each color
        ...
        if isfield(V1trials,'vis')
            ...
            figure; plot(combinedData); hold on;
            combinedAverage.vis.svdUV.(names{rr}){animal}=nanmean(combinedData,2);
            plot(combinedAverage.vis.svdUV.(names{rr}){animal},'k','LineWidth',4); title('Change in DFF'); hold off;
            figure;  plot(combinedAverage.vis.svdUV.(names{rr}){animal},'k'); title('Change in DFF');
            ...
            preStimAverage=combinedAverage.vis.svdUV.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_Pre.vis.svdUV.(names{rr}){animal}=min(preStimAverage);
            ...
            preStimAverage=combinedAverage.vis.pixUV.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_Pre.vis.pixUV.(names{rr}){animal}=min(preStimAverage);
            ...
            preStimAverage=combinedAverage.vis.uncorrected.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_Pre.vis.uncorrected.(names{rr}){animal}=min(preStimAverage);
            minAveFlour.vis.uncorrected.(names{rr}){animal}=min(postStimAverage);
            
            %for ZScore
            combinedData=cell2mat(V1trials_ZScore.vis.svdUV.(names{rr})); %combine across session
            figure; plot(combinedData); hold on;
            combinedAverage_ZScore.vis.svdUV.(names{rr}){animal}=nanmean(combinedData,2);
            plot(combinedAverage_ZScore.vis.svdUV.(names{rr}){animal},'k','LineWidth',4); title('ZScore'); hold off;
              figure;  plot(combinedAverage_ZScore.vis.svdUV.(names{rr}){animal},'k'); title('Change in DFF');
            postStimAverage=combinedAverage_ZScore.vis.svdUV.(names{rr}){animal}(((params.preEventWin*params.fsimaging)+1):(params.postEventWin+params.preEventWin)*params.fsimaging);
            preStimAverage=combinedAverage_ZScore.vis.svdUV.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_ZScore_Pre.vis.svdUV.(names{rr}){animal}=min(preStimAverage);
            minAveFlour_ZScore.vis.svdUV.(names{rr}){animal}=min(postStimAverage);
            
            combinedData=cell2mat(V1trials_ZScore.vis.pixUV.(names{rr})); %combine across session
            combinedAverage_ZScore.vis.pixUV.(names{rr}){animal}=nanmean(combinedData,2);
            postStimAverage=combinedAverage_ZScore.vis.pixUV.(names{rr}){animal}(((params.preEventWin*params.fsimaging)+1):(params.postEventWin+params.preEventWin)*params.fsimaging);
            preStimAverage=combinedAverage_ZScore.vis.pixUV.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_ZScore_Pre.vis.pixUV.(names{rr}){animal}=min(preStimAverage);
            minAveFlour_ZScore.vis.pixUV.(names{rr}){animal}=min(postStimAverage);
            
            combinedData=cell2mat(V1trials_ZScore.vis.uncorrected.(names{rr})); %combine across session
            combinedAverage_ZScore.vis.uncorrected.(names{rr}){animal}=nanmean(combinedData,2);
            postStimAverage=combinedAverage_ZScore.vis.uncorrected.(names{rr}){animal}(((params.preEventWin*params.fsimaging)+1):(params.postEventWin+params.preEventWin)*params.fsimaging);
            preStimAverage=combinedAverage_ZScore.vis.uncorrected.(names{rr}){animal}(1:(params.baselineWin)*params.fsimaging);
            minAveFlour_ZScore_Pre.vis.uncorrected.(names{rr}){animal}=min(preStimAverage);
            minAveFlour_ZScore.vis.uncorrected.(names{rr}){animal}=min(postStimAverage);
        end
        %% for airpuffs
...
if isfield(minAveFlour,'vis')
    for rr=1:length(names)% for each color
        ...
        set(gca,'TickDir','out');title(strcat('NegativeFlourescenceDips-VisualStim-DFF-',names{rr}));
        ...
        [~,finalData.vis.data.stats.(names{rr}).posthocPVal(1)]=ttest(finalData.vis.data.(names{rr})(:,1),finalData.vis.data.(names{rr})(:,2));
        [~,finalData.vis.data.stats.(names{rr}).posthocPVal(2)]=ttest(finalData.vis.data.(names{rr})(:,2),finalData.vis.data.(names{rr})(:,3));
        [~,finalData.vis.data.stats.(names{rr}).posthocPVal(3)]=ttest(finalData.vis.data.(names{rr})(:,1),finalData.vis.data.(names{rr})(:,3));
    end
end

% organize data and make paired plots for visual stim ZScore
if isfield(minAveFlour_ZScore,'vis')
    for rr=1:length(names)% for each color
        finalData_ZScore.vis.data.(names{rr})=[cell2mat(minAveFlour_ZScore.vis.uncorrected.(names{rr}))',cell2mat(minAveFlour_ZScore.vis.pixUV.(names{rr}))',cell2mat(minAveFlour_ZScore.vis.svdUV.(names{rr}))'];
        figure1=figure;
        plot(finalData_ZScore.vis.data.(names{rr})','-ko','MarkerFaceColor','k'); hold on;
        currMean=mean(finalData_ZScore.vis.data.(names{rr}),1);
        plot(currMean,'+','MarkerSize',30);  %draw markers at median
        xlim([0.7 3]);xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
        box off; ylabel('ZScore');
        set(gca,'TickDir','out');title(strcat('NegativeFlourescenceDips-VisualStim-ZScore-',names{rr}));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-ZScore-',names{rr})));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-ZScore',names{rr},'.pdf')));
        
        %do one way repeated measures anova followed by posthoc t-tests
        t = table(finalData_ZScore.vis.data.(names{rr})(:,1),finalData_ZScore.vis.data.(names{rr})(:,2),finalData_ZScore.vis.data.(names{rr})(:,3),...
            'VariableNames',{'Method1','Method2','Method3'});
        Method = table([1 2 3]','VariableNames',{'Measurements'});
        RM = fitrm(t,'Method1-Method3~1','WithinDesign',Method);
        finalData_ZScore.vis.data.stats.(names{rr}).RM= ranova(RM);
        finalData_ZScore.vis.data.stats.(names{rr}).posthocPVal=nan(1,size(finalData_ZScore.vis.data.(names{rr}),2));
        [~,finalData_ZScore.vis.data.stats.(names{rr}).posthocPVal(1)]=ttest(finalData_ZScore.vis.data.(names{rr})(:,1),finalData_ZScore.vis.data.(names{rr})(:,2));
        [~,finalData_ZScore.vis.data.stats.(names{rr}).posthocPVal(2)]=ttest(finalData_ZScore.vis.data.(names{rr})(:,2),finalData_ZScore.vis.data.(names{rr})(:,3));
        [~,finalData_ZScore.vis.data.stats.(names{rr}).posthocPVal(3)]=ttest(finalData_ZScore.vis.data.(names{rr})(:,1),finalData_ZScore.vis.data.(names{rr})(:,3));
    end
end

% organize data and make paired plots for visual stim pre stim
if isfield(minAveFlour_Pre,'vis')
    for rr=1:length(names)% for each color
        finalData_Pre.vis.data.(names{rr})=[cell2mat(minAveFlour_Pre.vis.uncorrected.(names{rr}))',cell2mat(minAveFlour_Pre.vis.pixUV.(names{rr}))',cell2mat(minAveFlour_Pre.vis.svdUV.(names{rr}))'];
        figure1=figure;
        plot(finalData_Pre.vis.data.(names{rr})','-ko','MarkerFaceColor','k'); hold on;
        currMean=mean(finalData_Pre.vis.data.(names{rr}),1);
        plot(currMean,'+','MarkerSize',30);  %draw markers at median
        xlim([0.7 3]);xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
        box off; ylabel('MinimumDiffDFF');
        set(gca,'TickDir','out');title(strcat('NegativeFlourescenceDips-VisualStim-DFF-PreStim-',names{rr}));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-PreStim-',names{rr})));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-PreStim-',names{rr},'.pdf')));
        
        %do one way repeated measures anova followed by posthoc t-tests
        t = table(finalData_Pre.vis.data.(names{rr})(:,1),finalData_Pre.vis.data.(names{rr})(:,2),finalData_Pre.vis.data.(names{rr})(:,3),...
            'VariableNames',{'Method1','Method2','Method3'});
        Method = table([1 2 3]','VariableNames',{'Measurements'});
        RM = fitrm(t,'Method1-Method3~1','WithinDesign',Method);
        finalData_Pre.vis.data.stats.(names{rr}).RM= ranova(RM);
    end
end

% organize data and make paired plots for visual stim ZScore pre stim
if isfield(minAveFlour,'vis')
    for rr=1:length(names)% for each color
        finalData_ZScore_Pre.vis.data.(names{rr})=[cell2mat(minAveFlour_ZScore_Pre.vis.uncorrected.(names{rr}))',cell2mat(minAveFlour_ZScore_Pre.vis.pixUV.(names{rr}))',cell2mat(minAveFlour_ZScore_Pre.vis.svdUV.(names{rr}))'];
        figure1=figure;
        plot(finalData_ZScore_Pre.vis.data.(names{rr})','-ko','MarkerFaceColor','k'); hold on;
        currMean=mean(finalData_ZScore_Pre.vis.data.(names{rr}),1);
        plot(currMean,'+','MarkerSize',30);  %draw markers at median
        xlim([0.7 3]);xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
        box off; ylabel('ZScore');
        set(gca,'TickDir','out');title(strcat('NegativeFlourescenceDips-VisualStim-ZScore-PreStim-',names{rr}));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-ZScore-PreStim-',names{rr})));
        saveas(figure1, fullfile(figuresFolder,strcat('NegativeFlourescenceDips-VisualStimPairedPlots-ZScore-PreStim-',names{rr},'.pdf')));
        
        %do one way repeated measures anova followed by posthoc t-tests
        t = table(finalData_ZScore_Pre.vis.data.(names{rr})(:,1),finalData_ZScore_Pre.vis.data.(names{rr})(:,2),finalData_ZScore_Pre.vis.data.(names{rr})(:,3),...
            'VariableNames',{'Method1','Method2','Method3'});
        Method = table([1 2 3]','VariableNames',{'Measurements'});
        RM = fitrm(t,'Method1-Method3~1','WithinDesign',Method);
        finalData_ZScore_Pre.vis.data.stats.(names{rr}).RM= ranova(RM);
        
    end
end
%average across mice
figure; plot(nanmean(cell2mat(combinedAverage.vis.svdUV.green),2)); ylabel('Change in DFF'); title('VisStim-AnimalAverage'); 
figure; plot(nanmean(cell2mat(combinedAverage_ZScore.vis.svdUV.green),2)); ylabel('ZScore'); title('VisStim-AnimalAverage');
...
if isfield(minAveFlour,'airpuff')
    for rr=1:length(names)% for each color
        ...
        [~,finalData.airpuff.data.stats.(names{rr}).posthocPVal(1)]=ttest(finalData.airpuff.data.(names{rr})(:,1),finalData.airpuff.data.(names{rr})(:,2));
        [~,finalData.airpuff.data.stats.(names{rr}).posthocPVal(2)]=ttest(finalData.airpuff.data.(names{rr})(:,2),finalData.airpuff.data.(names{rr})(:,3));
        [~,finalData.airpuff.data.stats.(names{rr}).posthocPVal(3)]=ttest(finalData.airpuff.data.(names{rr})(:,1),finalData.airpuff.data.(names{rr})(:,3));
