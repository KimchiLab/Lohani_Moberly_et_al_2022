...
figuresFolder='F:\GRABS_Data\Analyzed_SVDMethodPatch14\FiguresRevisionsFeb2022\Hemodynamics\VisualStimMaxSignalMethodComparison\DualMice';%where figures and output will be saved
...
for animal=1:length(MiceAnalyze)
    ...
    for rr=1:length(names)% for each color
        ...
        if isfield(V1trials,'vis')
            ...
            minAveFlour.vis.svdUV.(names{rr}){animal}=max(postStimAverage);
            ...
            minAveFlour.vis.pixUV.(names{rr}){animal}=max(postStimAverage);
            ...
            minAveFlour.vis.uncorrected.(names{rr}){animal}=max(postStimAverage);
        end
        %% for airpuffs
        if isfield(V1trials,'airpuff')
            ...
            minAveFlour.airpuff.svdUV.(names{rr}){animal}=max(postStimAverage);
            ...
            minAveFlour.airpuff.pixUV.(names{rr}){animal}=max(postStimAverage);
            ...
            minAveFlour.airpuff.uncorrected.(names{rr}){animal}=max(postStimAverage);
        end
    end
end
...
if isfield(minAveFlour,'vis')
    for rr=1:length(names)% for each color
        ...
        xlim([0.7 3]); ylim([-0.1 1]); xticks(1:3); xticklabels([{'Uncorrected'},{'Pixelwise-UV'},{'SpatialRegres-UV'}]); xtickangle(90); hold on;
        box off; ylabel('MaxDiffDFF');
        set(gca,'TickDir','out');title(strcat('Maxflourescence-VisualStim-',names{rr}));
        saveas(figure1, fullfile(figuresFolder,strcat('MaxFlourescence-VisualStimPairedPlots-',names{rr})));
        saveas(figure1, fullfile(figuresFolder,strcat('MaxFlourescence-VisualStimPairedPlots-',names{rr},'.pdf')));
...
if isfield(minAveFlour,'airpuff')
    for rr=1:length(names)% for each color
        ...
        set(gca,'TickDir','out');title(strcat('MaxFlourescence-Airpuff-',names{rr}));
        saveas(figure1, fullfile(figuresFolder,strcat('MaxFlourescence-AirpuffPairedPlots-',names{rr})));
        saveas(figure1, fullfile(figuresFolder,strcat('MaxFlourescence-AirpuffPairedPlots-',names{rr},'.pdf')));
